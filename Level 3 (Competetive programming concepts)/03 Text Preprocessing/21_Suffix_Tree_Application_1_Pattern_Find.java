...
class Suffix_Tree_Application_1_Pattern_Find {
  public static void main(String args[]) {
    ...
    ArrayList<Integer> ans = st.findPattern(st.root, pat, 0);
  ...
  static class SuffixTree {
    ...
    public ArrayList<Integer> findPattern(SuffixNode node, String pat, int i){
        ...
        for(SuffixNode child : node.child){
            ArrayList<Integer> nextAns = findPattern(child, pat, i);
            for(int x : nextAns){
                myAns.add(x);
            }
        }
    ...
    private ArrayList<Integer> leafNodes(SuffixNode node) {
        ArrayList<Integer> myAns = new ArrayList<>();
        if(node == null){
            return myAns;
        }
        ...
        for(SuffixNode child : node.child){
            ArrayList<Integer> nextAns = leafNodes(child);
            for(int val:nextAns){
                myAns.add(val);
            }
        }
