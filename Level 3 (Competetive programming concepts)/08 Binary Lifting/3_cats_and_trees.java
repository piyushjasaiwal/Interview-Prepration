/*abstract
You are given a rooted tree with N nodes labelled 1 through N. For each non-root node i, let's denote the direct parent of this node by pi.

N cats, also labelled 1 through N, have vanished from Chef's home. The cats are frightened, so they want to climb the tree one by one, in the order C1,C2,…,CN. At any time, each node may contain at most one cat. When cat c wants to start climbing the tree, the following happens:
...
The second line contains N space-separated integers p1,p2,…,pN. The parent of the root will be denoted with a 0.
The third line contains N space-separated integers C1,C2,…,CN.
Output
Print N lines. For each valid i, the i-th of these lines should contain a single integer — the number of jumps performed by cat Ci.

Constraints
1≤N≤3⋅105
0≤pi≤N for each valid i
C1,C2,…,CN form a permutation of 1,2,…,N
...
class cats_and_trees{
    ...
    public static int[][] ancestor_table_maker(int [] parent){
        int [][] ancestor = new int[20][parent.length];
        for(int i = 0;i<ancestor.length;i++){
            for(int j = 0;j<ancestor[0].length;j++){
                if(i == 0 && j == 0){
                    ancestor[i][j] = 0;
                }else if(i == 0){
                    ancestor[i][j] = parent[j];
                }else if(j == 0){
                    ancestor[i][j] = 0;
                }else{
