...
class Main {
  ...
  public static void printKLevelsDown(Node node, int k){
    ...
    printKLevelsDown_helper(node, k, 0);
  }

  public static void printKLevelsDown_helper(Node node, int k, int level){
  	if(node == null){
  		return ;
  	}
  	...
  	printKLevelsDown_helper(node.left, k, level+1);
  	printKLevelsDown_helper(node.right, k, level+1);
  }
