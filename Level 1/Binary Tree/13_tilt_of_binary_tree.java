...
class Main {
  ...
  public static int tilt(Node node){
    ...
    return tilt_helper(node, map);

  }

  public static int tilt_helper(Node node, HashMap<Node, Integer> map){
    if(node == null){
      return 0;
    }

    int sum_left = sum(node.left, map);
    int sum_right = sum(node.right, map);

    int t = Math.abs(sum_right-sum_left);
    tilt += t;
    tilt_helper(node.left, map);
    tilt_helper(node.right, map);
    return t;
  }

  public static int sum(Node node, HashMap<Node, Integer> map){
    if(node == null){
      return 0;
    }
    ...
    int sum = sum(node.left, map) + sum(node.right, map) + node.data;
    map.put(node, sum);
    return sum; 
  }
