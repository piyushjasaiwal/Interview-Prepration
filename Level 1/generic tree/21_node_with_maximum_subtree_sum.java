...
class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    pair p = new pair(-1, Integer.MIN_VALUE);
    max_sum_node(root, p);
    System.out.println(p.val+"@"+p.sum);
  }

  public static int max_sum_node(Node node, pair p){
    if(node == null){
      return 0;
    }

    int sum_curr = node.data;
    for(Node nn : node.children){
      sum_curr += max_sum_node(nn, p);
    }
